...
import static iota.CLConstants.Category.*;
...
abstract class CLInstruction {
    ...
    public static final CLInsInfo[] instructionInfo = {
            ...
            new CLInsInfo(TABLESWITCH, "tableswitch", DYNAMIC, IRRELEVANT, -1, FLOW_CONTROL3),
            new CLInsInfo(LOOKUPSWITCH, "lookupswitch", DYNAMIC, IRRELEVANT, -1, FLOW_CONTROL4),
            ...
            new CLInsInfo(INVOKESPECIAL, "invokespecial", 2, IRRELEVANT, DYNAMIC, METHOD1),
            new CLInsInfo(INVOKESTATIC, "invokestatic", 2, IRRELEVANT, DYNAMIC, METHOD1),
            new CLInsInfo(INVOKEINTERFACE, "invokeinterface", 4, IRRELEVANT, DYNAMIC, METHOD1),
            new CLInsInfo(INVOKEDYNAMIC, "invokedynamic", 2, IRRELEVANT, DYNAMIC, METHOD1),
     ...
     * @return true if the opcode is valid, and false otherwise.
     */
     ...
     * @param byteNum the byte to return (1: lower - 4: higher).
...
class CLObjectInstruction extends CLInstruction {
     ...
     * @param index  index into the constant pool, the item at which identifies the object.
     */
...
class CLFieldInstruction extends CLInstruction {
     ...
     * @param opcode     the opcode for this instruction.
     * @param pc         index of this instruction within the code array of a method.
     * @param index      index into the constant pool, the item at which contains the name and
...
class CLArithmeticInstruction extends CLInstruction {
    // Whether this instruction is preceded by a WIDE instruction; applies only to IINC.
...
class CLFlowControlInstruction extends CLInstruction {
    // Jump label; this flow control instruction will jump to an instruction after this label.
    private String jumpToLabel;

    // jumpLabel is resolved to this offset.
    private int jumpToOffset;

    // Index of the local variable containing the return address; applies only to RET instruction.
    private int index;
    ...
    // Key and offset (resolved labels from matchLabelPairs) table for LOOKUPSWITCH instruction.
     ...
     *                     values included.
     */
    public CLFlowControlInstruction(int opcode, int pc, String defaultLabel, int low, int high,
